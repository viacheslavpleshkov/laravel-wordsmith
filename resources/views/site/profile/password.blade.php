		...
		<div class="row">
			...
			<form action="{{ route('profile.password',$main->id) }}" method="POST">
				...
				<div>
					...
					<input type="password" class="full-width" name="password" placeholder="{{ __('site.profile-enter-password') }}" required>
				</div>

				<div class="form-group">
					<label>{{ __('site.profile-confirm-password') }}</label>
					<input type="password" class="full-width" name="password_confirmation" placeholder="{{ __('site.profile-enter-confirm-password') }}" required>
				</div>
